...
export class SlitherlinkGenerator {
	generate(width: number, height: number): SimpleSlitherlinkPuzzle {
		// Phase 1: Generate a valid solution loop
		const solution = this.generateValidLoop(width, height)

		// Calculate all cell numbers from the solution
		const cellNumbers = this.calculateAllCellNumbers(solution, width, height)

		return { width, height, solution, cellNumbers }
	}

	private generateValidLoop(width: number, height: number): SimpleSlitherlinkPuzzle['solution'] {
	...
	private calculateAllCellNumbers(
